...
def _load_all_results(voltage_level="MV"):
...
def _build_load_template(net):
    ...
    static_loads = net.load[net.load["profile"].str.startswith("mv_add", na=False)].copy()  # to remain untouched
    update_candidates = net.load[~net.load["profile"].str.startswith("mv_add", na=False)].copy()  # mv grid loads
...
def _make_mv_summary_dict(df: pd.DataFrame) -> dict:
    """
    ...
    out.update({
        ...
        "LV_total_reinforcement_cost":          float(df["LV_total_reinforcement_cost"].sum()),
        "LV_total_trafo_reinforcement_cost":    float(df["LV_total_trafo_reinforcement_cost"].sum()),
        "LV_total_line_reinforcement_cost":     float(df["LV_total_line_reinforcement_cost"].sum()),
        "MV_total_reinforcement_cost":          float(df["MV_total_reinforcement_cost"].sum()),
        "MV_total_trafo_reinforcement_cost":    float(df["MV_total_trafo_reinforcement_cost"].sum()),
        "MV_total_line_reinforcement_cost":     float(df["MV_total_line_reinforcement_cost"].sum()),
        "LV_base_#_of_transformers":            int(df["LV_base_#_of_transformers"].sum()),
        "LV_scen_#_of_crit_transformers":       int(df["LV_scen_#_of_crit_transformers"].sum()),
        "LV_scen_#_of_transformers":            int(df["LV_scen_#_of_transformers"].sum()),
        "LV_base_km_of_lines":                  float(df["LV_base_km_of_lines"].sum()),
        "LV_scen_km_of_crit_lines":             float(df["LV_scen_km_of_crit_lines"].sum()),
        "LV_scen_new_km_of_lines":              float(df["LV_scen_new_km_of_lines"].sum()),
        "MV_base_#_of_transformers":            int(df["MV_base_#_of_transformers"].sum()),
        "MV_scen_#_of_crit_transformers":       int(df["MV_scen_#_of_crit_transformers"].sum()),
        "MV_scen_#_of_transformers":            int(df["MV_scen_#_of_transformers"].sum()),
        "MV_base_km_of_lines":                  float(df["MV_base_km_of_lines"].sum()),
        "MV_scen_km_of_crit_lines":             float(df["MV_scen_km_of_crit_lines"].sum()),
        "MV_scen_new_km_of_lines":              float(df["MV_scen_new_km_of_lines"].sum()),
    })
    return out

def _hv_assign_load_to_network(results_dict, load_template, static_loads, load_input, scen_idx
    ...
    for idx, row in template.iterrows():
        ...
        rr = {"hv_bus": row["bus"], "grid": grid, **rr}
        dict_rows.append(rr)
    ...
    mv_reinforcement_dict = _make_mv_summary_dict(df_dict)

    hh_index = pd.DataFrame(bus_rows)

    return netload_new, mv_reinforcement_dict, hh_index, static_loads_subgrids
...
# ----------------------------
# Main HV reinforcement loop
# ----------------------------

def _reinforce_hv_grid(
        net, grid_code, mv_results_dict, load_input, load_template, static_loads, scen_idx, seed
        ):
    ...
    # Replace loads 
    netload_new, mv_reinforcement_dict, hh_data, static_loads_subgrids = _hv_assign_load_to_network(mv_results_dict, load_template, static_loads, load_input, scen_idx)
    ...
    (t_cost, l_cost, trafo_load, crit_trafos,
        crit_lines_km) = reinforce_grid._run_reinforcement(net, grid_code, netload_new, "HV")
    ...
    return (trafo_cost, line_cost, topMW,
            ...
            max_crit_lines_km, mv_reinforcement_dict, hh_data, static_loads_subgrids)
...
def run_scenarios(
    ...
    mv_results_dict = _load_all_results(voltage_level="MV")
    ...
    for tariff in tariff_space:
        ...
        for i, (hp, ev, pv) in enumerate(scenario_space):
            ...
            # import load data
            load_input = _extract_field_df(mv_results_dict, scenario=(hp, ev, pv), tariffs=tariff, fields=["scenario_load", "non_hh_load"])

            (trafo_cost, line_cost, topMW,
              max_trafo_load, max_crit_trafo, max_crit_lines_km, 
              mv_reinforcement_dict, hh_data, static_loads_subgrids) = _reinforce_hv_grid(net, grid_code, mv_results_dict, load_input, load_template, static_loads, scen_idx, seed)
            ...
            reinforcement_result = {
                ...
                'EV_percentage': ev,
                'PV_percentage': pv,
                'total_households': mv_reinforcement_dict["total_households"],
                'LV_total_reinforcement_cost': mv_reinforcement_dict["LV_total_reinforcement_cost"],
                'LV_total_trafo_reinforcement_cost': mv_reinforcement_dict["LV_total_trafo_reinforcement_cost"],
                'LV_total_line_reinforcement_cost': mv_reinforcement_dict["LV_total_line_reinforcement_cost"],
                'MV_total_reinforcement_cost': mv_reinforcement_dict["MV_total_reinforcement_cost"],
                'MV_total_trafo_reinforcement_cost': mv_reinforcement_dict["MV_total_trafo_reinforcement_cost"],
                'MV_total_line_reinforcement_cost': mv_reinforcement_dict["MV_total_line_reinforcement_cost"],
                'HV_total_reinforcement_cost': total_reinforcement_cost,
                ...
                'LV_base_#_of_transformers': mv_reinforcement_dict["LV_base_#_of_transformers"],
                'LV_scen_#_of_crit_transformers': mv_reinforcement_dict["LV_scen_#_of_crit_transformers"],
                'LV_scen_#_of_transformers': mv_reinforcement_dict["LV_scen_#_of_transformers"],
                'LV_base_km_of_lines': mv_reinforcement_dict["LV_base_km_of_lines"],
                'LV_scen_km_of_crit_lines': mv_reinforcement_dict["LV_scen_km_of_crit_lines"],
                'LV_scen_new_km_of_lines': mv_reinforcement_dict["LV_scen_new_km_of_lines"],
                'MV_base_#_of_transformers': mv_reinforcement_dict["MV_base_#_of_transformers"],
                'MV_scen_#_of_crit_transformers': mv_reinforcement_dict["MV_scen_#_of_crit_transformers"],
                'MV_scen_#_of_transformers': mv_reinforcement_dict["MV_scen_#_of_transformers"],
                'MV_base_km_of_lines': mv_reinforcement_dict["MV_base_km_of_lines"],
                'MV_scen_km_of_crit_lines': mv_reinforcement_dict["MV_scen_km_of_crit_lines"],
                'MV_scen_new_km_of_lines': mv_reinforcement_dict["MV_scen_new_km_of_lines"],
                'HV_base_#_of_transformers': base_trafo_count,
                'HV_scen_#_of_crit_transformers': max_crit_trafo,
                'HV_scen_#_of_transformers': len(net.trafo),
                'HV_base_km_of_lines': base_line_km,
                'HV_scen_km_of_crit_lines': max_crit_lines_km,
                'HV_scen_new_km_of_lines': net.line["length_km"].sum()
            }
